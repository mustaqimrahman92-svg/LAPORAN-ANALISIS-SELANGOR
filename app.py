...
# 2. Fungsi Sambungan Direct HTTP ke OpenAI (Tanpa Keperluan Library External)
def ringkas_guna_chatgpt_direct(teks_asal, kategori, api_key):
    prompt_arahan = f"""
...
if menu == "📝 Masukkan Laporan (Auto-AI)":
    ...
    # Input OpenAI API Key di Sidebar
    with st.sidebar:
        st.markdown("---")
        ...
        if not openai_api_key:
            ...
            with st.spinner("ChatGPT sedang menganalisis dan meringkaskan laporan mengikut format intel..."):
                try:
                    ...
                    # 2. Call ChatGPT via HTTP Request
                    hasil_ringkasan_ai = ringkas_guna_chatgpt_direct(teks_laporan_asal, kategori_pilihan, openai_api_key)
                    ...
                    st.error(f"Ralat semasa memproses laporan: {e}")
